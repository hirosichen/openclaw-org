...
* **技能命令：** `user-invocable` 技能會公開為斜線命令。名稱經過消毒為 `a-z0-9_`（最多 32 字元）；衝突時附加數字後綴（例如 `_2`）。
...
* **Slack：** `channels.slack.slashCommand` 仍然支援單一 `/openclaw` 風格命令。若你啟用 `commands.native`，必須為每個內建命令建立一個 Slack 斜線命令（與 `/help` 相同的名稱）。
